...
@section("breadcrumb","Edit Dosen")

@section("content")
<div class="space-y-6">
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Edit Dosen</h1>
            <p class="mt-1 text-xs text-slate-500">Perbarui data dosen pembimbing & penguji</p>
        </div>
    ...
    <div class="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200">
        <form method="POST" action="{{ route('admin.master.dosen.update', $dosen) }}" class="w-full space-y-4">
            @csrf
            @method('PUT')
            <div>
                ...
                <input name="nidn" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" required placeholder="Contoh: 0012058001" value="{{ old('nidn', $dosen->nidn) }}">
            ...
            <div>
                ...
                <input name="nama" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" required placeholder="Nama tanpa gelar" value="{{ old('nama', $dosen->nama) }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="gelar_depan" placeholder="Dr." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" value="{{ old('gelar_depan', $dosen->gelar_depan) }}">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Gelar Belakang</label>
                    <input name="gelar_belakang" placeholder="M.T." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" value="{{ old('gelar_belakang', $dosen->gelar_belakang) }}">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="prodi_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" required>
                    ...
                    @foreach($prodiList as $p)
                        <option value="{{ $p->id }}" {{ old('prodi_id', $dosen->prodi_id) == $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input name="jabatan" placeholder="Lektor / Asisten Ahli / Guru Besar" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" value="{{ old('jabatan', $dosen->jabatan) }}">
            </div>
            ...
            <div>
                ...
                <input type="email" name="email" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" required placeholder="email@ummu.ac.id" value="{{ old('email', $dosen->email) }}">
