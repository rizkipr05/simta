...
@section("breadcrumb","Tambah Program Studi")

@section("content")
<div class="space-y-6">
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Tambah Program Studi</h1>
            <p class="mt-1 text-xs text-slate-500">Tambah data program studi baru di Fakultas Teknik UMMU</p>
        </div>
    ...
    <div class="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200">
        <form method="POST" action="{{ route('admin.master.program-studi.store') }}" class="w-full space-y-5">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input name="kode" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" required placeholder="Contoh: TI" value="{{ old('kode') }}">
                ...
                <div>
                    ...
                    <input name="nama" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" required placeholder="Contoh: Teknik Informatika" value="{{ old('nama') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input name="jenjang" value="{{ old('jenjang', 'S1') }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition">
                </div>

                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Nama Kaprodi</label>
                    <input name="kaprodi" placeholder="Nama Kepala Program Studi" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-sm font-medium transition" value="{{ old('kaprodi') }}">
                </div>
            </div>
